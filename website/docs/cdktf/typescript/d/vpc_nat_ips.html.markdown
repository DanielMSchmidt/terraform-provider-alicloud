...
* `nameRegex` - (Optional, ForceNew) A regex string to filter results by Nat Ip name.
...
* `natIpCidr` - (Required, ForceNew) NAT IP ADDRESS of the address segment.
* `natIpName` - (Optional, ForceNew) NAT IP ADDRESS the name of the root directory. Length is from `2` to `128` characters, must start with a letter or the Chinese at the beginning can contain numbers, half a period (.), underscore (_) and dash (-). But do not start with `http://` or `https://` at the beginning.
...
<!-- cache-key: cdktf-0.17.0-pre.15 input-b78fe76c33b9150979397c9d9260d4a952dcd8b47556f9d6a5fd709ed0ffcd19 -->