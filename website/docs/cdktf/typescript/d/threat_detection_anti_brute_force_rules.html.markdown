...
* `antiBruteForceRuleNames` - (Optional, ForceNew) The name of the Anti-Brute Force Rule. You can specify at most 10 names.
* `nameRegex` - (Optional, ForceNew) A regex string to filter results by the name of the defense rule.
...
* `rules` - A list of Anti Brute Force Rule Entries. Each element contains the following attributes:
    ...
    * `uuidList` - An array consisting of the UUIDs of servers to which the defense rule is applied.

<!-- cache-key: cdktf-0.17.0-pre.15 input-7d5d69c7ed0bdbb72f6ab45ec62e901d2dae1e2bf684e2027d097e477614f3cb -->