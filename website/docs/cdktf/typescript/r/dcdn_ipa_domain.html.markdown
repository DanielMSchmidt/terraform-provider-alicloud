...
class MyConvertedCode extends cdktf.TerraformStack {
  constructor(scope: constructs.Construct, name: string) {
    ...
    new alicloud.dcdnIpaDomain.DcdnIpaDomain(this, "example", {
...
* `domainName` - (Required, ForceNew) The domain name to be added to IPA. Wildcard domain names are supported. A wildcard domain name must start with a period (.).
* `resourceGroupId` - (Optional, Computed) The ID of the resource group. If you do not set this parameter, the system automatically assigns the ID of the default resource group.
...
* `id` - The resource ID in terraform of Ipa Domain. Its value is same as `domainName`.
