...
* `authKey` - (Optional) The authentication key of the BGP group.
* `bgpGroupName` - (Optional) The name of the BGP group. The name must be `2` to `128` characters in length and can contain digits, periods (.), underscores (_), and hyphens (-). The name must start with a letter but cannot start with `http://` or `https://`.
...
* `isFakeAsn` - (Optional, Computed) The is fake asn. A router that runs BGP typically belongs to only one AS. In some cases, for example, the AS needs to be migrated or is merged with another AS, a new AS number replaces the original one.
* `localAsn` - (Optional, Computed) The AS number on the Alibaba Cloud side.
* `peerAsn` - (Required) The AS number of the BGP peer.
* `routerId` - (Required, ForceNew) The ID of the VBR.
...
```
<!-- cache-key: cdktf-0.17.0-pre.15 input-5268b72e92b4b1d728cdf82eaa8a62e7b6bef199cc35e12f7d9b377983a996c1 -->