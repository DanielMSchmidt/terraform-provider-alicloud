...
* `cacheDiskCategory` - (Optional, Computed, ForceNew) The cache disk type. Valid values: `cloudEfficiency`, `cloudSsd`.
* `cacheDiskSizeInGb` - (Required) size of the cache disk. Unit: `gb`. The upper limit of the basic gateway cache disk is `1` TB (`1024` GB), that of the standard gateway is `2` TB (`2048` GB), and that of other gateway cache disks is `32` TB (`32768` GB). The lower limit for the file gateway cache disk capacity is `40` GB, and the lower limit for the block gateway cache disk capacity is `20` GB.
...
```
<!-- cache-key: cdktf-0.17.0-pre.15 input-31e83fbc64994582c0a18a8c666ff5367f468e40336be773bbefb3d8a8231a69 -->