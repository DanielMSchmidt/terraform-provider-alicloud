...
class MyConvertedCode extends cdktf.TerraformStack {
  constructor(scope: constructs.Construct, name: string) {
    super(scope, name);
    new alicloud.kvstoreAuditLogConfig.KvstoreAuditLogConfig(this, "example", {
      dbAudit: true,
...
* `dbAudit` - (Optional) Indicates Whether to Enable the Audit Log.  Valid value: 
...
* `instanceId` - (Required, ForceNew) Instance ID, Call the Describeinstances Get.
...
* `id` - The resource ID in terraform of Audit Log Config. Its value is same as `instanceId`.
