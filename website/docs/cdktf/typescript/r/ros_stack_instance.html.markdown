...
* `operationDescription` - (Optional) The operation description.
* `operationPreferences` - (Optional) The operation preferences. The operation settings. The following fields are supported:
...
* `parameterOverrides` - (Optional, Sensitive) ParameterOverrides. See the following `Block parameter_overrides`.
* `stackInstanceAccountId` - (Required) The account to which the stack instance belongs.
* `stackInstanceRegionId` - (Required) The region of the stack instance.
* `stackGroupName` - (Required, ForceNew) The name of the stack group.
* `retainStacks` - (Optional) Specifies whether to retain the stack corresponding to the stack instance.Default value `false`. **NOTE:** When `retainStacks` is `true`, the stack is retained. If the stack is retained, the corresponding stack is not deleted when the stack instance is deleted from the stack group. 
* `timeoutInMinutes` - (Optional) The timeout period that is specified for the stack creation request. Default value: `60`. Unit: `minutes`.

#### Block parameter_overrides

The parameter_overrides supports the following: 

* `parameterKey` - (Required, Sensitive) The key of override parameter. If you do not specify the key and value of the parameter, ROS uses the key and value that you specified when you created the stack group.
* `parameterValue` - (Required, Sensitive) The value of override parameter. If you do not specify the key and value of the parameter, ROS uses the key and value that you specified when you created the stack group.
...
```
<!-- cache-key: cdktf-0.17.0-pre.15 input-247470d8c93319ad9a7c6cbb7d85de9dce46c9dbc860b55dfcd8a7abebff3149 -->