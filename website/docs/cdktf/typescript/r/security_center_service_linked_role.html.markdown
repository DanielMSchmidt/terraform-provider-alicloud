...
class MyConvertedCode extends cdktf.TerraformStack {
  constructor(scope: constructs.Construct, name: string) {
    super(scope, name);
    new alicloud.securityCenterServiceLinkedRole.SecurityCenterServiceLinkedRole(
      this,
      "service_linked_role",
      {}
    );
...
* `id` - The ID of the DataSource. The value is same as `productName`. Valid Value: `aliyunServiceRolePolicyForSas`.
