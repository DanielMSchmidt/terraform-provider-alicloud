...
* `controlPolicyName` - (Required) The name of control policy.
* `description` - (Optional) The description of control policy.
* `effectScope` - (Required, ForceNew) The effect scope. Valid values `ram`.
* `policyDocument` - (Required) The policy document of control policy.
