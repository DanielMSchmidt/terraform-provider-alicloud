...
* `enableAdminAccess` - (Optional, ForceNew, Computed) Whether to enable public network access.
* `enableInternetAccess` - (Optional, ForceNew, Computed) Whether to grant local administrator rights to users who use cloud desktops.
* `ramDirectoryName` - (Required, ForceNew) The name of the directory. The name must be 2 to 255 characters in length. It must start with a letter but cannot start with `http://` or `https://`. It can contain letters, digits, colons (:), underscores (_), and hyphens (-).
...
```
<!-- cache-key: cdktf-0.17.0-pre.15 input-a0d02a352428ae76ed6adaf141604d3d58b9eecc853adc564c4a241fbff28872 -->