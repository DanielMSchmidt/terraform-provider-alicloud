...
* `autoDeleteState` - (Optional, ForceNew) Specifies whether the Auto Delete attribute is configured. Valid values:
...
* `autoExpireState` - (Optional) The validity period after which the queue is automatically deleted.
  If the queue is not accessed within a specified period of time, it is automatically deleted.
* `deadLetterExchange` - (Optional) The dead-letter exchange. A dead-letter exchange is used to receive rejected messages. 
...
* `deadLetterRoutingKey` - (Optional) The dead letter routing key.
* `exclusiveState` - (Optional, ForceNew) Specifies whether the queue is an exclusive queue. Valid values:
...
* `maxLength` - (Optional) The maximum number of messages that can be stored in the queue.
  If this threshold is exceeded, the earliest messages that are routed to the queue are discarded.
* `maximumPriority` - (Optional) The highest priority supported by the queue. This parameter is set to a positive integer.
  Valid values: 0 to 255. Recommended values: 1 to 10
* `messageTtl` - (Optional) The message TTL of the queue.
...
* `queueName` - (Required, ForceNew) The name of the queue.
