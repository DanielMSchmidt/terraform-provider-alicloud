...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_alb_listeners_ids =
        alicloud.data_alicloud_alb_listeners.DataAlicloudAlbListeners(self, "ids",
            ids=["example_id"]
        )
        cdktf.TerraformOutput(self, "alb_listener_id_1",
            value=cdktf.property_access(data_alicloud_alb_listeners_ids.listeners, ["0", "id"
            ])
        )
```
