...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_ecs_auto_snapshot_policies_example =
        alicloud.data_alicloud_ecs_auto_snapshot_policies.DataAlicloudEcsAutoSnapshotPolicies(self, "example",
            ids=["sp-bp14e66xxxxxxxx"],
            name_regex="tf-testAcc"
        )
        cdktf.TerraformOutput(self, "first_ecs_auto_snapshot_policy_id",
            value=cdktf.property_access(data_alicloud_ecs_auto_snapshot_policies_example.policies, ["0", "id"])
        )
```
