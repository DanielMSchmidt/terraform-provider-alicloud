...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_eip_addresses_example =
        alicloud.data_alicloud_eip_addresses.DataAlicloudEipAddresses(self, "example",
            ids=["eip-bp1jvx5ki6c********"],
            name_regex="the_resource_name"
        )
        cdktf.TerraformOutput(self, "first_eip_address_id",
            value=cdktf.property_access(data_alicloud_eip_addresses_example.addresses, ["0", "id"
            ])
        )
```
