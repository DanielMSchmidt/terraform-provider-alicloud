...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_direct_mail_receiverses_example =
        alicloud.data_alicloud_direct_mail_receiverses.DataAlicloudDirectMailReceiverses(self, "example",
            ids=["ca73b1e4fb0df7c935a5097a****"],
            name_regex="the_resource_name"
        )
        cdktf.TerraformOutput(self, "first_direct_mail_receivers_id",
            value=cdktf.property_access(data_alicloud_direct_mail_receiverses_example.receiverses, ["0", "id"])
        )
```
