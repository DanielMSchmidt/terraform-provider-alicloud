...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_db_instance_classes_resources =
        alicloud.data_alicloud_db_instance_classes.DataAlicloudDbInstanceClasses(self, "resources",
            engine="MySQL",
            engine_version="5.6",
            instance_charge_type="PostPaid",
            output_file="./classes.txt"
        )
        cdktf.TerraformOutput(self, "first_db_instance_class",
            value=cdktf.property_access(data_alicloud_db_instance_classes_resources.instance_classes, ["0", "instance_class"])
        )
```
...
* `category` - (Optional) DB Instance category. the value like [`Basic`, `HighAvailability`, `Finance`, `AlwaysOn`, `serverless_basic`, `serverless_standard`, `serverless_ha`, `cluster`], [detail info](https://www.alibabacloud.com/help/doc-detail/69795.htm).
