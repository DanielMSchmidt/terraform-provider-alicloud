...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_oos_executions_example =
        alicloud.data_alicloud_oos_executions.DataAlicloudOosExecutions(self, "example",
            ids=["execution_id"],
            status="Success",
            template_name="name"
        )
        cdktf.TerraformOutput(self, "first_execution_id",
            value=cdktf.property_access(data_alicloud_oos_executions_example.executions, ["0", "id"
            ])
        )
```
