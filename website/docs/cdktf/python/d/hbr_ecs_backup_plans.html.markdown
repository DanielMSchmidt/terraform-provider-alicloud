...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_hbr_ecs_backup_plans_ids =
        alicloud.data_alicloud_hbr_ecs_backup_plans.DataAlicloudHbrEcsBackupPlans(self, "ids",
            name_regex="plan-name"
        )
        cdktf.TerraformOutput(self, "hbr_ecs_backup_plan_id",
            value=cdktf.property_access(data_alicloud_hbr_ecs_backup_plans_ids.plans, ["0", "id"
            ])
        )
```
