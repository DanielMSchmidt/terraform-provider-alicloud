...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_oss_buckets_oss_buckets_ds =
        alicloud.data_alicloud_oss_buckets.DataAlicloudOssBuckets(self, "oss_buckets_ds",
            name_regex="sample_oss_bucket"
        )
        cdktf.TerraformOutput(self, "first_oss_bucket_name",
            value=cdktf.property_access(data_alicloud_oss_buckets_oss_buckets_ds.buckets, ["0", "name"
            ])
        )
```
