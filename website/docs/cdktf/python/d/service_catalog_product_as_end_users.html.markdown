...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_service_catalog_product_as_end_users_default =
        alicloud.data_alicloud_service_catalog_product_as_end_users.DataAlicloudServiceCatalogProductAsEndUsers(self, "default",
            name_regex="ram模板创建"
        )
        cdktf.TerraformOutput(self, "alicloud_service_catalog_product_as_end_user_example_id",
            value=cdktf.property_access(data_alicloud_service_catalog_product_as_end_users_default.users, ["0", "id"])
        )
```
