...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_cs_kubernetes_clusters_k8_sClusters =
        alicloud.data_alicloud_cs_kubernetes_clusters.DataAlicloudCsKubernetesClusters(self, "k8s_clusters",
            kube_config_file_prefix="~/.kube/k8s",
            name_regex="my-first-k8s",
            output_file="my-first-k8s-json"
        )
        cdktf.TerraformOutput(self, "output",
            value=data_alicloud_cs_kubernetes_clusters_k8_sClusters.clusters
        )
```
