...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        alicloud_key_pair_default = alicloud.key_pair.KeyPair(self, "default",
            key_name="keyPairDatasource"
        )
        data_alicloud_key_pairs_default =
        alicloud.data_alicloud_key_pairs.DataAlicloudKeyPairs(self, "default_1",
            name_regex=cdktf.Token.as_string(alicloud_key_pair_default.key_name)
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        data_alicloud_key_pairs_default.override_logical_id("default")
```
