...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_cen_bandwidth_packages_example =
        alicloud.data_alicloud_cen_bandwidth_packages.DataAlicloudCenBandwidthPackages(self, "example",
            instance_id="cen-id1",
            name_regex="^foo"
        )
        cdktf.TerraformOutput(self, "first_cen_bandwidth_package_id",
            value=cdktf.property_access(data_alicloud_cen_bandwidth_packages_example.packages, ["0", "id"])
        )
```
