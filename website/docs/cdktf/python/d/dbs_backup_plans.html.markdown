...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_dbs_backup_plans_ids =
        alicloud.data_alicloud_dbs_backup_plans.DataAlicloudDbsBackupPlans(self, "ids")
        data_alicloud_dbs_backup_plans_name_regex =
        alicloud.data_alicloud_dbs_backup_plans.DataAlicloudDbsBackupPlans(self, "nameRegex",
            name_regex="^my-BackupPlan"
        )
        cdktf.TerraformOutput(self, "dbs_backup_plan_id_1",
            value=cdktf.property_access(data_alicloud_dbs_backup_plans_ids.plans, ["0", "id"
            ])
        )
        cdktf.TerraformOutput(self, "dbs_backup_plan_id_2",
            value=cdktf.property_access(data_alicloud_dbs_backup_plans_name_regex.plans, ["0", "id"
            ])
        )
```
