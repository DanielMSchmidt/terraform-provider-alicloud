...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_vpc_public_ip_address_pools_ids =
        alicloud.data_alicloud_vpc_public_ip_address_pools.DataAlicloudVpcPublicIpAddressPools(self, "ids",
            ids=["example_id"]
        )
        data_alicloud_vpc_public_ip_address_pools_name_regex =
        alicloud.data_alicloud_vpc_public_ip_address_pools.DataAlicloudVpcPublicIpAddressPools(self, "nameRegex",
            name_regex="example_name"
        )
        cdktf.TerraformOutput(self, "vpc_public_ip_address_pool_id_1",
            value=cdktf.property_access(data_alicloud_vpc_public_ip_address_pools_ids.pools, ["0", "id"])
        )
        cdktf.TerraformOutput(self, "vpc_public_ip_address_pool_id_2",
            value=cdktf.property_access(data_alicloud_vpc_public_ip_address_pools_name_regex.pools, ["0", "id"])
        )
```
