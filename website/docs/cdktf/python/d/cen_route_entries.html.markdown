...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_cen_route_entries_entry =
        alicloud.data_alicloud_cen_route_entries.DataAlicloudCenRouteEntries(self, "entry",
            instance_id="cen-id1",
            route_table_id="vtb-id1"
        )
        cdktf.TerraformOutput(self, "first_route_entries_route_entry_cidr_block",
            value=cdktf.property_access(data_alicloud_cen_route_entries_entry.entries, ["0", "cidr_block"
            ])
        )
```
