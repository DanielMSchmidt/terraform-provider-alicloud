...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_ecs_launch_templates_example =
        alicloud.data_alicloud_ecs_launch_templates.DataAlicloudEcsLaunchTemplates(self, "example",
            ids=["lt-bp1a469uxxxxxx"],
            name_regex="your_launch_name"
        )
        cdktf.TerraformOutput(self, "first_ecs_launch_template_id",
            value=cdktf.property_access(data_alicloud_ecs_launch_templates_example.templates, ["0", "id"])
        )
```
