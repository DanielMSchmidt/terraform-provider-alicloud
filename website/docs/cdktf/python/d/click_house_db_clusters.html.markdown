...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_click_house_db_clusters_default =
        alicloud.data_alicloud_click_house_db_clusters.DataAlicloudClickHouseDbClusters(self, "default_1",
            ids=[cdktf.Token.as_string(alicloud_click_house_db_cluster_default.id)]
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        data_alicloud_click_house_db_clusters_default.override_logical_id("default")
        cdktf.TerraformOutput(self, "db_cluster",
            value=cdktf.property_access(data_alicloud_click_house_db_clusters_default.ids, ["0"
            ])
        )
```
