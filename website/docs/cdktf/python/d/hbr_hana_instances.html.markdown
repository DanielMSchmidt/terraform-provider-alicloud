...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_hbr_hana_instances_ids =
        alicloud.data_alicloud_hbr_hana_instances.DataAlicloudHbrHanaInstances(self, "ids",
            ids=["example_id"]
        )
        cdktf.TerraformOutput(self, "hbr_hana_instance_id_1",
            value=cdktf.property_access(data_alicloud_hbr_hana_instances_ids.instances, ["0", "id"
            ])
        )
```
