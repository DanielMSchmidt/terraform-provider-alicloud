...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_resource_manager_control_policies_example =
        alicloud.data_alicloud_resource_manager_control_policies.DataAlicloudResourceManagerControlPolicies(self, "example",
            ids=["example_value"],
            name_regex="the_resource_name"
        )
        cdktf.TerraformOutput(self, "first_resource_manager_control_policy_id",
            value=cdktf.property_access(data_alicloud_resource_manager_control_policies_example.policies, ["0", "id"])
        )
```
