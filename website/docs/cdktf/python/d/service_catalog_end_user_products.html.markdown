...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_service_catalog_end_user_products_default =
        alicloud.data_alicloud_service_catalog_end_user_products.DataAlicloudServiceCatalogEndUserProducts(self, "default",
            name_regex="ram模板创建"
        )
        cdktf.TerraformOutput(self, "alicloud_service_catalog_end_user_product_example_id",
            value=cdktf.property_access(data_alicloud_service_catalog_end_user_products_default.end_user_products, ["0", "id"])
        )
```
