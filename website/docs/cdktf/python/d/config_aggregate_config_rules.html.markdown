...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_config_aggregate_config_rules_example =
        alicloud.data_alicloud_config_aggregate_config_rules.DataAlicloudConfigAggregateConfigRules(self, "example",
            aggregator_id="ca-3a9b626622af001d****",
            ids=["cr-5154626622af0034****"],
            name_regex="the_resource_name"
        )
        cdktf.TerraformOutput(self, "first_config_aggregate_config_rule_id",
            value=cdktf.property_access(data_alicloud_config_aggregate_config_rules_example.rules, ["0", "id"])
        )
```
