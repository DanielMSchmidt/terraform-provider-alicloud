...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_cloud_firewall_vpc_firewalls_default =
        alicloud.data_alicloud_cloud_firewall_vpc_firewalls.DataAlicloudCloudFirewallVpcFirewalls(self, "default",
            ids=["id-example"],
            member_uid="1415189284827022",
            status="closed",
            vpc_firewall_name="tf-test"
        )
        cdktf.TerraformOutput(self, "alicloud_cfw_vpc_firewall_example_id",
            value=cdktf.property_access(data_alicloud_cloud_firewall_vpc_firewalls_default.firewalls, ["0", "id"])
        )
```
