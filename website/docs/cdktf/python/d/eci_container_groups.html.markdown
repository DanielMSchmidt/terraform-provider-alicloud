...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_eci_container_groups_example =
        alicloud.data_alicloud_eci_container_groups.DataAlicloudEciContainerGroups(self, "example",
            ids=["example_value"]
        )
        cdktf.TerraformOutput(self, "first_eci_container_group_id",
            value=cdktf.property_access(data_alicloud_eci_container_groups_example.groups, ["0", "id"])
        )
```
