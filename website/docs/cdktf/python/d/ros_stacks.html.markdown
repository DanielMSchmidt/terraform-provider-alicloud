...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_ros_stacks_example =
        alicloud.data_alicloud_ros_stacks.DataAlicloudRosStacks(self, "example",
            ids=["example_value"],
            name_regex="the_resource_name"
        )
        cdktf.TerraformOutput(self, "first_ros_stack_id",
            value=cdktf.property_access(data_alicloud_ros_stacks_example.stacks, ["0", "id"
            ])
        )
```
