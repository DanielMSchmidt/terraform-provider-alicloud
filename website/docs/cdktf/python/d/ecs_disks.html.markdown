...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_ecs_disks_example =
        alicloud.data_alicloud_ecs_disks.DataAlicloudEcsDisks(self, "example",
            ids=["d-artgdsvdvxxxx"],
            name_regex="tf-test"
        )
        cdktf.TerraformOutput(self, "first_ecs_disk_id",
            value=cdktf.property_access(data_alicloud_ecs_disks_example.disks, ["0", "id"
            ])
        )
```
