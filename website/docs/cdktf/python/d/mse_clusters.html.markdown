...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_mse_clusters_example =
        alicloud.data_alicloud_mse_clusters.DataAlicloudMseClusters(self, "example",
            ids=["mse-cn-0d9xxxx"],
            status="INIT_SUCCESS"
        )
        cdktf.TerraformOutput(self, "cluster_id",
            value=cdktf.property_access(data_alicloud_mse_clusters_example.clusters, ["0", "id"
            ])
        )
```
