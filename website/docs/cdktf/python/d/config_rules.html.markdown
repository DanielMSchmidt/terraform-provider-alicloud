...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_config_rules_example =
        alicloud.data_alicloud_config_rules.DataAlicloudConfigRules(self, "example",
            ids=["cr-ed4bad756057********"],
            name_regex="tftest"
        )
        cdktf.TerraformOutput(self, "first_config_rule_id",
            value=cdktf.property_access(data_alicloud_config_rules_example.rules, ["0", "id"
            ])
        )
```
