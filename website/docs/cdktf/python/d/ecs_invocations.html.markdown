...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_ecs_invocations_ids =
        alicloud.data_alicloud_ecs_invocations.DataAlicloudEcsInvocations(self, "ids",
            ids=["example-id"]
        )
        cdktf.TerraformOutput(self, "ecs_invocation_id_1",
            value=cdktf.property_access(data_alicloud_ecs_invocations_ids.invocations, ["0", "id"
            ])
        )
```
