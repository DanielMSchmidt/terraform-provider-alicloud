...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_cen_route_services_example =
        alicloud.data_alicloud_cen_route_services.DataAlicloudCenRouteServices(self, "example",
            cen_id="cen-7qthudw0ll6jmc****"
        )
        cdktf.TerraformOutput(self, "first_cen_route_service_id",
            value=cdktf.property_access(data_alicloud_cen_route_services_example.services, ["0", "id"])
        )
```
