...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_adb_db_cluster_lake_versions_ids =
        alicloud.data_alicloud_adb_db_cluster_lake_versions.DataAlicloudAdbDbClusterLakeVersions(self, "ids",
            ids=["example_id"]
        )
        cdktf.TerraformOutput(self, "adb_db_cluster_lake_version_id_1",
            value=cdktf.property_access(data_alicloud_adb_db_cluster_lake_versions_ids.versions, ["0", "id"])
        )
```
