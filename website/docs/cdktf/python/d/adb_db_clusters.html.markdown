...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_adb_db_clusters_example =
        alicloud.data_alicloud_adb_db_clusters.DataAlicloudAdbDbClusters(self, "example",
            description_regex="example"
        )
        cdktf.TerraformOutput(self, "first_adb_db_cluster_id",
            value=cdktf.property_access(data_alicloud_adb_db_clusters_example.clusters, ["0", "id"
            ])
        )
```
