...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_privatelink_vpc_endpoint_services_example =
        alicloud.data_alicloud_privatelink_vpc_endpoint_services.DataAlicloudPrivatelinkVpcEndpointServices(self, "example",
            ids=["example_value"],
            name_regex="the_resource_name"
        )
        cdktf.TerraformOutput(self, "first_privatelink_vpc_endpoint_service_id",
            value=cdktf.property_access(data_alicloud_privatelink_vpc_endpoint_services_example.services, ["0", "id"])
        )
```
