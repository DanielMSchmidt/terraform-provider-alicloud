...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_dcdn_domains_example =
        alicloud.data_alicloud_dcdn_domains.DataAlicloudDcdnDomains(self, "example",
            ids=["example.com"]
        )
        cdktf.TerraformOutput(self, "domain_id",
            value=cdktf.property_access(data_alicloud_dcdn_domains_example.domains, ["0", "id"
            ])
        )
```
