...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        data_alicloud_alb_system_security_policies_defaults =
        alicloud.data_alicloud_alb_system_security_policies.DataAlicloudAlbSystemSecurityPolicies(self, "defaults",
            ids=["tls_cipher_policy_1_0"]
        )
        cdktf.TerraformOutput(self, "alb_system_security_policy_id_1",
            value=cdktf.property_access(data_alicloud_alb_system_security_policies_defaults.policies, ["0", "id"])
        )
```
