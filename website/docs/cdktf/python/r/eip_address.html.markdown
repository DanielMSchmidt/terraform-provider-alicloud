...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        alicloud.eip_address.EipAddress(self, "example",
...
* `security_protection_types` - (Optional, ForceNew, Available in v1.184.0+) The edition of Anti-DDoS. If you do not set this parameter, Anti-DDoS is basic level. If you set the value to `AntiDDoS_Enhanced`, High capacity Anti-DDoS Origin is enabled.
* `instance_charge_type` - (Optional, ForceNew, Deprecated from v1.126.0+) Field `instance_charge_type` has been deprecated from provider version 1.126.0, and it will be removed in the future version. Please use the new attribute `payment_type` instead.
* `name` - (Optional, Computed, Deprecated from v1.126.0+) Field `name` has been deprecated from provider version 1.126.0, and it will be removed in the future version. Please use the new attribute `address_name` instead.
