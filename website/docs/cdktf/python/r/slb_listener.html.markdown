...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        slb_listener_name = cdktf.TerraformVariable(self, "slb_listener_name",
            default="forSlbListener"
        )
        alicloud_slb_acl_listener = alicloud.slb_acl.SlbAcl(self, "listener",
            ip_version="ipv4",
            name=slb_listener_name.string_value
        )
        alicloud.slb_acl_entry_attachment.SlbAclEntryAttachment(self, "first",
            acl_id=cdktf.Token.as_string(alicloud_slb_acl_listener.id),
            comment="first",
            entry="10.10.10.0/24"
        )
        alicloud.slb_acl_entry_attachment.SlbAclEntryAttachment(self, "second",
            acl_id=cdktf.Token.as_string(alicloud_slb_acl_listener.id),
            comment="second",
            entry="168.10.10.0/24"
        )
        alicloud_slb_load_balancer_listener =
        alicloud.slb_load_balancer.SlbLoadBalancer(self, "listener_4",
        ...
        alicloud_slb_load_balancer_listener.override_logical_id("listener")
        alicloud_slb_listener_listener = alicloud.slb_listener.SlbListener(self, "listener_5",
            acl_id=cdktf.Token.as_string(alicloud_slb_acl_listener.id),
            acl_status="on",
            acl_type="white",
            backend_port=80,
            ...
            healthy_threshold=8,
            idle_timeout=30,
            load_balancer_id=cdktf.Token.as_string(alicloud_slb_load_balancer_listener.id),
            protocol="http",
            request_timeout=80,
            sticky_session="on",
            sticky_session_type="insert",
            unhealthy_threshold=8,
            x_forwarded_for=SlbListenerXForwardedFor(
                retrive_slb_id=True,
                retrive_slb_ip=True
            )
        )
        # This allows the Terraform resource name to match the original name. You can remove the call if you don't need them to match.
        alicloud_slb_listener_listener.override_logical_id("listener")
```
