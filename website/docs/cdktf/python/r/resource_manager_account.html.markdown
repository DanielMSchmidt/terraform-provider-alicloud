...
class MyConvertedCode(cdktf.TerraformStack):
    def __init__(self, scope, name):
        ...
        alicloud_resource_manager_folder_f1 =
        alicloud.resource_manager_folder.ResourceManagerFolder(self, "f1",
            folder_name="test1"
        )
        alicloud.resource_manager_account.ResourceManagerAccount(self, "example",
            display_name="RDAccount",
            folder_id=cdktf.Token.as_string(alicloud_resource_manager_folder_f1.id)
        )
```
